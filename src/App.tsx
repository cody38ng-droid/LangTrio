...
const BreakGame = ({ setShowBreakGame, timeLeft }: { setShowBreakGame: (show: boolean) => void, timeLeft: number }) => {
  ...
  const update = () => {
    ...
    setSnake(prev => {
      ...
      // The mouse is relative to the screen, but the snake is in the arena
      // For simplicity, let's assume the screen is a window into the arena
      ...
      // NPC collision (Player hitting NPC)
    ...
    setNpcs(prevNpcs => {
      return prevNpcs.map(npc => {
        // Simple AI: occasionally change direction or move towards food
  ...
  useEffect(() => {
    ...
    // Draw "Out of Bounds" indicator
    ctx.fillStyle = '#0f172a';
    ctx.fillRect(-5000, -5000, 10000, 5000); // Top
    ctx.fillRect(-5000, ARENA_SIZE, 10000, 5000); // Bottom
    ctx.fillRect(-5000, 0, 5000, ARENA_SIZE); // Left
    ctx.fillRect(ARENA_SIZE, 0, 5000, ARENA_SIZE); // Right
    ...
    // Draw Arena Border
    ctx.strokeStyle = '#f43f5e'; // Rose color for boundary
    ctx.lineWidth = 15;
    ctx.lineJoin = 'round';
      ...
      >
        ...
        {gameState === 'START' && (
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm z-30 flex flex-col items-center justify-center text-white text-center p-6">
            ...
            <Button onClick={() => setGameState('PLAYING')} size="lg" className="rounded-full font-bold">
              Start Slithering
            </Button>
          </div>
        )}
        ...
        {gameState === 'GAME_OVER' && (
          <div className="absolute inset-0 bg-red-500/60 backdrop-blur-md z-30 flex flex-col items-center justify-center text-white text-center p-6">
            ...
            <div className="flex gap-4">
              <Button onClick={resetGame} size="lg" className="rounded-full bg-white text-red-600 hover:bg-white/90 font-bold">
                Try Again
              </Button>
              <Button onClick={() => setShowBreakGame(false)} variant="outline" size="lg" className="rounded-full border-white text-white hover:bg-white/20">
                Back to Study
              </Button>
            </div>
          </div>
...
const ChineseRubyText = ({ text, pinyin }: { text: string; pinyin: string }) => {
  // Clean text of non-chinese characters for alignment if needed, 
  // but usually we want to keep punctuation.
  // We'll try to split pinyin by space and match with characters.
  const pinyinWords = pinyin.split(' ');
  const chars = text.split('');
  
  // This is a naive implementation. It assumes 1 pinyin word per character.
  // It's not perfect for multisyllabic words that might be one pinyin token,
  // but in our data, most are space-separated per character.
  
  let pIdx = 0;
  return (
    <div className="flex flex-wrap justify-center gap-x-4 gap-y-6">
      {chars.map((char, index) => {
        // If it's punctuation, don't give it pinyin
        if (/[，。！？“”：；（）、]/.test(char)) {
          return (
...
export default function App() {
  ...
  // Leaderboard data
  const DUMMY_LEADERBOARDS: Record<Language, { name: string; xp: number; avatar: string; isMe?: boolean }[]> = {
    English: [
